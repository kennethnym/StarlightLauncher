package kenneth.app.starlightlauncher.datetime

import kenneth.app.starlightlauncher.api.TemperatureUnit

internal val DEFAULT_DATE_TIME_VIEW_SIZE = DateTimeViewSize.NORMAL
internal val DEFAULT_WEATHER_UNIT = TemperatureUnit.CELSIUS
internal const val DEFAULT_SHOW_WEATHER = false
internal const val DEFAULT_USE_AUTO_LOCATION = false
internal const val DEFAULT_AUTO_LOCATION_CHECK_FREQUENCY = 3600000L
internal const val DEFAULT_WEATHER_CHECK_FREQUENCY = 3600000L
internal const val DEFAULT_USE_24HR_CLOCK = false
